...
class _ReportLostScreenState extends State<ReportLostScreen> {
  ...
  Future<void> _addPhotos() async {
    // TODO: pakai geolocator kalau mau isi lokasi otomatis dari GPS asli.
  ...
  void _useCurrentLocation() {
    // TODO: ganti dengan geolocator + reverse geocoding untuk lokasi asli.
  ...
  Widget build(BuildContext context) {
    ...
    final navy = const Color(0xFF1E3A8A);
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            ...
            const Text('Report Lost Item', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          ],
        ),
        actions: [
          Padding(
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('AI Smart Matching', style: theme.textTheme.titleSmall?.copyWith(color: navy, fontWeight: FontWeight.bold)),
            ...
            // --- Item Name & Model ---
            _FieldLabel(label: 'Item Name & Model', required: true, hint: 'Be specific'),
            ...
            _FieldLabel(label: 'Distinctive Features & Content', hint: '4 rows'),
            const SizedBox(height: 8),
            TextFormField(
            ...
            Row(
              ...
              children: [
                _FieldLabel(label: 'Location Lost', required: true),
                TextButton.icon(
                  ...
                  icon: Icon(Icons.my_location, size: 16, color: navy),
                  label: Text('Use current location', style: TextStyle(color: navy, fontSize: 12)),
                ),
              ],
            ),
            const SizedBox(height: 8),
            ...
            // --- When was it lost ---
            _FieldLabel(label: 'When was it lost?', required: true),
            const SizedBox(height: 8),
            Row(
              children: [
                Expanded(
            ...
            InkWell(
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    CircleAvatar(radius: 22, backgroundColor: navy.withOpacity(0.08), child: Icon(Icons.camera_alt_outlined, color: navy)),
...
class _FieldLabel extends StatelessWidget {
  const _FieldLabel({required this.label, this.required = false, this.hint});
  final String label;
  final bool required;
  final String? hint;
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        RichText(
          text: TextSpan(
            style: DefaultTextStyle.of(context).style.copyWith(fontWeight: FontWeight.w600, color: Colors.black87),
            children: [
              TextSpan(text: label),
        ...
        if (hint != null) Text(hint!, style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Colors.black45)),
      ],
    );
  }
}
...
class _NotifyTile extends StatelessWidget {
  const _NotifyTile({required this.icon, required this.title, required this.subtitle, required this.value, required this.onChanged});
  final IconData icon;
  final String title;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          CircleAvatar(radius: 18, backgroundColor: const Color(0xFF1E3A8A).withOpacity(0.08), child: Icon(icon, size: 18, color: const Color(0xFF1E3A8A))),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
